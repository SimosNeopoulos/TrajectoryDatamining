...
def k_cosine_similarity(main_trajectory, trajectories, embedding_dict,
                        node_index, k):

    similarity_list = []
    for trajectory in trajectories.values():
        similarity = get_cos_similarity(
            main_trajectory, trajectory, embedding_dict, node_index
        )
        similarity_list.append(
            ((main_trajectory.id, trajectory.id), similarity))
...
def get_distance_similarity(trajectory1, trajectory2, node_dict, pair_dist):
    distance12 = calculate_node_distance(trajectory1, trajectory2, node_dict,
                                         pair_dist)
    distance21 = calculate_node_distance(trajectory2, trajectory1, node_dict,
                                         pair_dist)
...
def k_distance_similarity(main_trajectory, trajectories, node_dict, pair_dist,
    ...
    similarity_list = []
    for trajectory in trajectories.values():
        similarity = get_distance_similarity(
            main_trajectory, trajectory, node_dict, pair_dist
        )
        similarity_list.append(
            ((main_trajectory.id, trajectory.id), similarity))
...
def calculate_node_distance(trajectory1, trajectory2, node_dict, pair_dist):
    distance_list = []
    for node1 in trajectory1.trajectory_path:
        distance_list.append(min([pair_dist[node_dict[node1.node_id]]
                                  [node_dict[node2.node_id]]
