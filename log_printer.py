...
def print_graph_name(graph_str, city, log_file):
...
def print_traj_info(traj_dict, log_file):
...
def get_cos_sim(main_traj, traj_dict, emb_dict, k, sim_method):
    start = timer()
    sim = sm.cos_sim_mesure(main_traj, traj_dict, emb_dict, k, sim_method)
    end = timer()

    return sim, end - start


def print_sim(emb_dict: dict, dist_data: list, traj_dict: dict, rand_traj: list, sim_method: str, log_file: str):
    ...
    for i in range(20):
        ...
        cos_sim, cos_thread_time = get_cos_sim(traj_dict[rand_traj[i]], traj_dict, emb_dict, k, sim_method)

        dist_time.append(dist_thread_time)

        # cos_sim = sm.cos_sim_mesure(
        #     traj_dict[rand_traj[i]], traj_dict,
        #     emb_dict, k, sim_method
        # )
